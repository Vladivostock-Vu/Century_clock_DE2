module controller_v2 (
    ...
    output reg mode,          // 0 = Giờ/Phút/Giây, 1 = Ngày/Tháng/Năm
    output reg [3:0] sel,
    ...
    // =========================================================
    // 3. LOGIC TIMER VÀ MODE
    // =========================================================
    wire tick_60s;
    
    timer_tick #(.CLK_FREQ(50000000), .SECONDS(60)) timer_inst (
        .clk(clk), .rst_n(~rst_clean), .en(~pause_sec), .tick_out(tick_60s)
    );
    
    timer_tick #(.CLK_FREQ(50000000), .SECONDS(1)) timer_1s_inst (
        .clk(clk), .rst_n(~rst_clean), .en(~pause_sec), .tick_out(tick_1s)
    );
